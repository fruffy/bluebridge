        ...
        for widget in self.widgetToItem:
            ...
            if 'Switch' in tags:
                ...
                if 'ip' in opts and len(opts['ip']) > 0:
                    ...
                    ipBaseNum, prefixLen = netParse( self.appPrefs['ipBase'] )
                    ip = ipAdd(i=nodeNum, prefixLen=prefixLen, ipBaseNum=ipBaseNum)
                hostDirs = [('./tmp/config', './tmp/'+name+'/var/config')]
                if 'privateDirectory' not in opts:
                    opts['privateDirectory'] = hostDirs
                else:
                    hostDirs.append(opts['privateDirectory'])

                # Create the correct host class
                if 'cores' in opts or 'cpu' in opts:
                    if 'privateDirectory' in opts:
                        hostCls = partial( CPULimitedHost,
                                           privateDirs=hostDirs )
                    else:
...
def configureHost(host, hostNum):
    ...
    host.cmd('echo ' + testString)
    host.cmd('ip address change dev h' + str(hostNum) +
                  '-eth0 scope global 0:0:01' + '{0:02x}'.format(hostNum) + '::/48')
    host.cmd('ip -6 route add local 0:0:0100::/40  dev h' +
                  str(hostNum) + '-eth0')
    host.cmd('ip -6 route add local 0:0:01' +
                  '{0:02x}'.format(hostNum) + '::/48 dev lo')
    host.cmd('xterm  -T \"server' + str(hostNum) +
