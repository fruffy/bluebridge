    ...
    for host in hosts:
        ...
        if (hostNum == 2):
            host.cmdPrint('xterm  -T \"thrift' + str(host)[1] +
                          '\" -e \"./thrift/tutorial_thrift/c_glib/thrift_remote_mem_test_server -c ./tmp/config/distMem.cnf; bash\" &')
        if (hostNum == 3):
            host.cmdPrint('xterm  -T \"thrift' + str(host)[1] +
                          '\" -e \"./thrift/tutorial_thrift/c_glib/thrift_simple_array_comp_server; bash\" &')
        hostNum += 1
